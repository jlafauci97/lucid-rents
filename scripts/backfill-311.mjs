#!/usr/bin/env node
/**
 * Backfill 311 complaints linking for a specific metro.
 * Usage: node scripts/backfill-311.mjs <metro>
 */
...
if (!metro) { console.error("Usage: node scripts/backfill-311.mjs <metro>"); process.exit(1); }
...
async function main() {
  ...
  if (!meta) { console.error(`Unknown metro: ${metro}`); process.exit(1); }

  console.log(`[${metro}] Loading buildings...`);
  ...
  console.log(`[${metro}] Loaded ${total.toLocaleString()} buildings, ${addrMap.size.toLocaleString()} addresses`);
  ...
  while (true) {
    const { data: batch, error } = await supabase.from("complaints_311")
      ...
      .eq("metro", metro).is("building_id", null)
      .not("incident_address", "is", null)
      .range(offset, offset + 5000 - 1);
    if (error) { console.error(`[${metro}] Fetch error:`, error.message); break; }
    ...
    const addrEntries = [...addrToKeys.entries()];

    async function processAddr([addr, keys]) {
      ...
      if (!bid && metro !== "nyc") {
        ...
        const { data: nb, error: ce } = await supabase.from("buildings").insert({
          full_address: `${addr}, ${meta.city.toUpperCase()}, ${meta.state}`,
          house_number: parts?.[1] || "", street_name: parts?.[2] || addr,
          city: meta.city, state: meta.state, metro, slug,
          violation_count: 0, complaint_count: 0, review_count: 0, overall_score: null,
        }).select("id").single();
        if (ce) {
          if (ce.code === "23505") {
            const { data: ex } = await supabase.from("buildings").select("id").eq("slug", slug).eq("metro", metro).single();
            if (ex) bid = ex.id;
          }
          if (!bid) { totalUnmatched++; return; }
    ...
    for (let i = 0; i < addrEntries.length; i += CONCURRENCY) {
      await Promise.all(addrEntries.slice(i, i + CONCURRENCY).map(processAddr));
    }

    totalProcessed += batch.length;
    if (totalProcessed % 25000 === 0) {
      console.log(`[${metro}] ${totalProcessed.toLocaleString()} processed, ${totalLinked.toLocaleString()} linked, ${totalCreated} created, ${totalUnmatched} unmatched`);
    }
    offset += 5000;
  }

  console.log(`\n[${metro}] DONE: ${totalProcessed.toLocaleString()} processed, ${totalLinked.toLocaleString()} linked, ${totalCreated} buildings created, ${totalUnmatched} unmatched`);
}
