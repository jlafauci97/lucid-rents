...
async function processFile(link, fi, total) {
  ...
  if (SKIP_CREATE) log(`  [${fi+1}/${total}] Skipping building creation (--skip-create)`);
  if (!SKIP_CREATE) {
    ...
    if (unmatchedBuildings.size > 0) {
      ...
      for (let i = 0; i < batches.length; i += BATCH) {
        ...
        const { data, error } = await supabase.from("buildings").insert(toInsert).select("id, slug, zip_code");
        if (error) {
          let batchCreated = 0;
