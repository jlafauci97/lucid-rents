...
def retry_supabase(fn, max_retries=3, backoff=5):
    """Retry a Supabase call on transient errors (502, 503, timeout, pool, schema cache)."""
    for attempt in range(max_retries):
        try:
            return fn()
        except Exception as e:
...
def upsert_rents(building_id: str, rent_by_beds: dict, unit_number: str | None = None) -> int:
    ...
    try:
        ...
        return -1  # negative signals history failed even though building_rents succeeded
        ...
        for page_num in range(1, max_pages + 1):
            ...
            for listing in listings:
                ...
                try:
                    ...
                    print(f"    {label} {addr} -> {abs(rents_added)} rents, {amenities_added} amenities, listing={'OK' if listing_saved else 'FAIL'}{unit_str}{history_status}")
                except Exception as e:
                    total_failed += 1
