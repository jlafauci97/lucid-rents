#!/usr/bin/env node
/**
 * Backfill Chicago RLTO (Residential Landlord Tenant Ordinance) violations.
 *
 * Data source: RLTO Violations (xgbn-v72z)
 * https://data.cityofchicago.org/resource/xgbn-v72z.json
 *
 * Usage:
 *   node scripts/backfill-chicago-rlto.mjs
 *   node scripts/backfill-chicago-rlto.mjs --offset=5000
 */
...
const ENDPOINT = "https://data.cityofchicago.org/resource/xgbn-v72z.json";
...
async function batchUpsert(rows) {
  ...
  for (let i = 0; i < rows.length; i += 500) {
    ...
    const { error, count } = await supabase.from("chicago_rlto_violations").upsert(batch, {
    ...
    if (error) {
      if (error.code === "23505") {
        for (const row of batch) {
          const { error: sErr } = await supabase.from("chicago_rlto_violations").upsert(row, {
            onConflict: "case_number",
          });
          if (!sErr) added++;
        }
...
async function main() {
  console.log(`\n=== Chicago RLTO Violations Backfill ===`);
  ...
  while (true) {
    ...
    const rows = records
      ...
      .map((r) => ({
        ...
        violation_description: r.violation_description || null,
  ...
  console.log(`\nDone! Total RLTO violations upserted: ${total}`);
}
