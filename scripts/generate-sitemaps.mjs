...
// back here AND in src/lib/sitemap/generator.ts to relaunch it.
const VALID_CITIES = ["nyc"];
...
// LA_ZIPS / CHICAGO_ZIPS / MIAMI_ZIPS / HOUSTON_ZIPS intentionally excluded
// while those metros are off the public site — ZIP_MAPS is iterated directly
// with Object.entries below.
const ZIP_MAPS = { nyc: NYC_ZIPS };
...
async function generateHubsSitemap() {
  ...
  if (VALID_CITIES.includes("chicago")) {
...
function rebuildIndex() {
  ...
  // rows, so the build — which knows exactly how many files it wrote — is the
  // authoritative source. Always exactly matches the files that exist.
  const buildingChunks = files.filter((f) => /^b-\d+\.xml$/.test(f)).length;
  const landlordChunks = files.filter((f) => /^l-\d+\.xml$/.test(f)).length;
  writeFileSync(
    "src/lib/sitemap/chunk-manifest.json",
    JSON.stringify(
      {
        _comment:
          "GENERATED by scripts/generate-sitemaps.mjs (rebuildIndex). Do not edit by hand. Records how many static sitemap chunk files live in public/sitemap/, so dedicated index routes (e.g. /sitemap-buildings.xml) can enumerate them without a slow DB COUNT or a filesystem readdir at runtime.",
        buildingChunks,
        landlordChunks,
