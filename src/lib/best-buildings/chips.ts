...
export const CHIPS: Record<ChipId, Chip> = {
  ...
  "most-reviewed": {
    ...
    long_description:
      "Buildings with at least 10 verified tenant reviews. More reviews = higher confidence in the LucidIQ score.",
    cities: "all",
    column_filters: [{ column: "review_count", op: "gte", value: 10 }],
  ...
  "no-violations": {
    ...
    long_description:
      "Buildings with zero open housing-code violations and at least 3 tenant reviews — the landlord is keeping up.",
    cities: "all",
    column_filters: [
      { column: "violation_count", op: "eq", value: 0 },
      { column: "review_count", op: "gte", value: 3 },
    ],
    sort: { column: "overall_score", ascending: false },
  ...
  "large-buildings": {
    ...
    description: "100+ unit buildings with full amenities.",
    long_description:
      "Bigger buildings, usually with doormen, elevators, and more amenities. Tradeoffs: less intimate, more noise on weekends.",
    cities: "all",
    column_filters: [{ column: "residential_units", op: "gte", value: 100 }],
