...
// ──────────────────────────────────────────────────────────────
// Per-section loaders — used by streaming wrappers in
// components/building/v2/streaming/*.tsx. Each loader covers the
// exact slice of BuildingV2Data consumed by its corresponding
// section. Running these in their own Promise lets Suspense stream
// each section independently as its query completes.
// ──────────────────────────────────────────────────────────────
...
export async function loadRentsData(
  ...
  current: BuildingV2Data["rents"]["current"];
...
export async function loadReviewsData(
  ...
  const [aggregate, pullQuotes] = await Promise.all([
    safe(async () => {
      ...
      total: 0,
      avgRating: 0,
      distribution: [1, 2, 3, 4, 5].map((s) => ({ stars: s as 1 | 2 | 3 | 4 | 5, count: 0, pct: 0 })),
    } as { total: number; avgRating: number; distribution: BuildingV2Data["reviews"]["distribution"] }),
    safe(async () => {
      const { data } = await supabase
        .from("reviews")
        .select("id, body, overall_rating, created_at, profile:profiles(display_name)")
  ...
  const [buyouts, scepInspections, retrofit] = await Promise.all([
    safe(async () => {
      const { data } = await supabase
        ...
        .order("disclosure_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["laData"]["buyouts"];
    }, [] as BuildingV2Data["laData"]["buyouts"]),
    safe(async () => {
      const { data } = await supabase
        .from("lahd_scep_inspections")
        .select("id, inspection_date, compliance_status, violations_found")
        ...
        .order("inspection_date", { ascending: false, nullsFirst: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["laData"]["scepInspections"];
    }, [] as BuildingV2Data["laData"]["scepInspections"]),
    safe(async () => {
      const { data } = await supabase
        .from("la_earthquake_retrofit")
        .select("retrofit_type, compliance_status, deadline, completion_date")
        .eq("building_id", buildingId)
        .limit(1);
      return (data ?? []) as Array<NonNullable<BuildingV2Data["laData"]["earthquakeRetrofit"]>>;
  ...
  const [rltoViolations, demolitions, leadInspections, affordableUnits, energy] = await Promise.all([
    safe(async () => {
      const { data } = await supabase
        ...
        .order("violation_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["chicagoData"]["rltoViolations"];
    }, [] as BuildingV2Data["chicagoData"]["rltoViolations"]),
    safe(async () => {
      const { data } = await supabase
        .from("chicago_demolitions")
        .select("id, permit_number, issue_date, status, work_description")
        ...
        .order("issue_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["chicagoData"]["demolitions"];
    }, [] as BuildingV2Data["chicagoData"]["demolitions"]),
    safe(async () => {
      const { data } = await supabase
        .from("chicago_lead_inspections")
        .select("id, inspection_date, result, risk_level")
        ...
        .order("inspection_date", { ascending: false, nullsFirst: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["chicagoData"]["leadInspections"];
    }, [] as BuildingV2Data["chicagoData"]["leadInspections"]),
    safe(async () => {
      const { data } = await supabase
        .from("chicago_affordable_units")
        .select("id, project_name, affordable_units, total_units")
        .eq("building_id", buildingId)
        .limit(10);
      return (data ?? []) as BuildingV2Data["chicagoData"]["affordableUnits"];
    }, [] as BuildingV2Data["chicagoData"]["affordableUnits"]),
    safe(async () => {
      const { data } = await supabase
        .from("energy_benchmarks")
        .select("energy_star_score, report_year, site_eui")
        ...
        .order("report_year", { ascending: false })
        .limit(1);
      return (data ?? []) as Array<NonNullable<BuildingV2Data["chicagoData"]["energyRating"]>>;
    }, [] as Array<NonNullable<BuildingV2Data["chicagoData"]["energyRating"]>>),
  ]);
  return {
    rltoViolations,
    demolitions,
  ...
  const [recerts, unsafeStructures, stormDamage, floodClaims] = await Promise.all([
    safe(async () => {
      const { data } = await supabase
        ...
        .order("due_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["miamiData"]["recerts"];
    }, [] as BuildingV2Data["miamiData"]["recerts"]),
    safe(async () => {
      const { data } = await supabase
        .from("miami_unsafe_structures")
        .select("id, case_number, violation_type, case_date, status")
        ...
        .order("case_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["miamiData"]["unsafeStructures"];
    }, [] as BuildingV2Data["miamiData"]["unsafeStructures"]),
    safe(async () => {
      const { data } = await supabase
        .from("miami_storm_damage")
        .select("id, disaster_name, disaster_date, damage_category, fema_verified_loss")
        ...
        .order("disaster_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["miamiData"]["stormDamage"];
    }, [] as BuildingV2Data["miamiData"]["stormDamage"]),
    safe(async () => {
      const { data } = await supabase
        .from("miami_flood_claims")
        .select("id, claim_date, flood_zone, amount_paid")
        ...
        .order("claim_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["miamiData"]["floodClaims"];
  ...
  const [dangerousBuildings, industrialProximity, taxProtests, affordableHousing] = await Promise.all([
    safe(async () => {
      const { data } = await supabase
        ...
        .order("case_date", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["houstonData"]["dangerousBuildings"];
    }, [] as BuildingV2Data["houstonData"]["dangerousBuildings"]),
    safe(async () => {
      const { data } = await supabase
        .from("houston_industrial_proximity")
        .select("id, facility_name, distance_miles, industry_type, total_releases_lbs")
        ...
        .order("distance_miles", { ascending: true })
        .limit(10);
      return (data ?? []) as BuildingV2Data["houstonData"]["industrialProximity"];
    }, [] as BuildingV2Data["houstonData"]["industrialProximity"]),
    safe(async () => {
      const { data } = await supabase
        .from("houston_tax_protests")
        .select("id, protest_year, original_value, final_value, reduction_pct")
        ...
        .order("protest_year", { ascending: false })
        .limit(10);
      return (data ?? []) as BuildingV2Data["houstonData"]["taxProtests"];
    }, [] as BuildingV2Data["houstonData"]["taxProtests"]),
    safe(async () => {
      const { data } = await supabase
        .from("houston_affordable_housing")
        .select("id, project_name, affordable_units, total_units")
        .eq("building_id", buildingId)
        .limit(10);
      return (data ?? []) as BuildingV2Data["houstonData"]["affordableHousing"];
