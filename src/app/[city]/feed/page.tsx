...
async function FeedStats({ city }: { city: import("@/lib/cities").City }) {
  ...
  const stats = [
    ...
    { icon: HardHat, label: "Building Violations", count: counts?.dob_violations_count ?? 0, color: "text-[#6366F1]", bg: "bg-blue-50" },
    { icon: Building2, label: "Buildings Tracked", count: counts?.buildings_count ?? 0, color: "text-[#1A1F36]", bg: "bg-gray-100" },
  ];

  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-[#E2E8F0]">
        <h3 className="text-sm font-bold text-[#1A1F36]">Data Snapshot</h3>
      </div>
        ...
        {stats.map((stat) => (
          <div key={stat.label} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1">
              <p className="text-xs text-[#5E6687]">{stat.label}</p>
...
async function TrendingBuildings({ city }: { city: import("@/lib/cities").City }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-[#E2E8F0]">
        <h3 className="text-sm font-bold text-[#1A1F36] flex items-center gap-1.5">
      ...
      <div className="divide-y divide-[#f1f5f9]">
        {buildings.map((b, i) => (
          <Link
            ...
            className="flex items-start gap-3 px-4 py-3 hover:bg-[#FAFBFD] transition-colors group"
          >
            <span className="text-xs font-bold text-[#A3ACBE] mt-0.5 w-4">{i + 1}</span>
            ...
            <ChevronRight className="w-4 h-4 text-[#e2e8f0] group-hover:text-[#6366F1] flex-shrink-0 mt-0.5 transition-colors" />
          </Link>
        ))}
      </div>
      <Link
        href={cityPath("/worst-rated-buildings", city as City)}
        className="block px-4 py-3 text-sm text-[#6366F1] hover:bg-[#EFF6FF] transition-colors font-medium border-t border-[#f1f5f9]"
      >
        View full rankings
      </Link>
    </div>
...
function FeedSourceLabel() {
  ...
  return (
    <div className="px-4 text-xs text-[#A3ACBE] space-y-1">
