  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <motion.span className="text-2xl font-black" style={{ color }} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", delay: 0.8 }}>
          {grade}
        </motion.span>
      </div>
    </div>
...
export function NeighborhoodSafetyCard({ crime }: { crime: CrimeSignal | null }) {
  if (!crime) return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 sm:p-8 shadow-sm">
      <p className="text-xs font-semibold tracking-[2px] uppercase text-gray-400 mb-3">Neighborhood Safety</p>
  ...
  const trendColor = crime.trend_label === "improving" ? "text-emerald-600" : crime.trend_label === "worsening" ? "text-red-500" : "text-gray-400";
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 sm:p-8 shadow-sm">
      {crime.trend_label === "worsening" && crime.yoy_violent_trend > 10 && (
        <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5 mb-5 text-[10px] text-red-600 font-medium">
          <AlertTriangle size={13} /> Violent crime +{Math.round(crime.yoy_violent_trend)}% YoY
        </div>
      )}
      <p className="text-xs font-semibold tracking-[2px] uppercase text-gray-400 mb-5">Neighborhood Safety</p>
      <div className="flex items-center gap-6">
        <GradeRing grade={crime.safety_grade} />
        <div className="flex-1">
          <p className="text-sm text-gray-600">{level} crime vs NYC</p>
          <div className={`flex items-center gap-1.5 text-xs font-medium ${trendColor} mt-1`}>
      ...
      <p className="text-[8px] text-gray-300 mt-5">NYC Open Data — NYPD (12mo)</p>
    </div>
  );
}
