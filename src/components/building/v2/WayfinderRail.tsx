...
export function WayfinderRail({ grade, buildingName, city, buildingPath, buildingId }: Props) {
  ...
  useEffect(() => {
    ...
    // Defer layout reads + write to a fresh rAF slot so we don't force a
    // synchronous reflow mid-render (PSI flagged this as a forced reflow).
  ...
  return (
    <aside className="wayfinder">
      ...
      <div className="tools">
        ...
        ><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" fill={saved ? "currentColor" : "none"}/></svg>{saved ? "Saved" : "Save"}</button>
        <button
          type="button"
          className="tool"
          onClick={() => {
