...
interface CommonIssuesProps {
  ...
  /** Stored counts from the buildings row — used to message empty state when records aren't loaded */
  totalViolations?: number;
  totalComplaints?: number;
}

export function CommonIssues({ topViolations, topComplaints, totalViolations, totalComplaints }: CommonIssuesProps) {
  const showViolations = topViolations.length > 0 || (totalViolations ?? 0) > 0;
  const showComplaints = topComplaints.length > 0 || (totalComplaints ?? 0) > 0;
  if (!showViolations && !showComplaints) return null;
        ...
        <div className="rounded-xl border p-4 shadow-sm" style={{ backgroundColor: T.surface, borderColor: T.border }}>
          ...
          {topViolations.length > 0 ? (
            ...
            <p className="text-xs" style={{ color: T.text3 }}>
              {(totalViolations ?? 0).toLocaleString()} on file — type breakdown not available.
            </p>
          )}
        </div>
      )}
      ...
      {showComplaints && (
        <div className="rounded-xl border p-4 shadow-sm" style={{ backgroundColor: T.surface, borderColor: T.border }}>
          ...
          {topComplaints.length > 0 ? (
            ...
            <p className="text-xs" style={{ color: T.text3 }}>
              {(totalComplaints ?? 0).toLocaleString()} on file — type breakdown not available.
            </p>
          )}
        </div>
      )}
