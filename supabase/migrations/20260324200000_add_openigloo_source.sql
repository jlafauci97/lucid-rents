-- Add 'openigloo' as a valid source for building_rents, building_amenities, unit_rent_history
...
ALTER TABLE building_rents ADD CONSTRAINT building_rents_source_check
  CHECK (source IN ('streeteasy', 'zillow', 'rent_com', 'columbia_rcp', 'apartmentratings', 'redfin', 'compass', 'zumper', 'openigloo'));
...
ALTER TABLE building_listings ADD CONSTRAINT building_listings_source_check
  CHECK (source IN ('rent_com', 'streeteasy', 'zillow', 'columbia_rcp', 'apartmentratings', 'redfin', 'compass', 'zumper', 'openigloo'));
...
-- unit_rent_history (may not have a source check, add one if missing)
DO $$
DECLARE
  cname text;
BEGIN
